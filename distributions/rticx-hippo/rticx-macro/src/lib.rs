...
use rticx_deadline_pass::{DeadlineToPriorityPass /* DeadlineToPriorityPassImpl */};
...
pub fn app(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    // use the standard deadline to priority pass provided bp the rticx-deadline-pass crate
    #[cfg(feature = "deadline-pass")]
...
impl CorePassBackend for HippoRtic {
        ...
        let initialize_dispatcher_interrupts =
            app_analysis.used_irqs.iter().map(|(irq_name, priority)| {
                ...
                quote! {
                    ...
                    rticx_hippo::export::enable(
                        rticx_hippo::export::interrupts::#irq_name,
                        #priority as u8,
                    );
                }
            });
    ...
    fn generate_interrupt_free_fn(&self, mut empty_body_fn: ItemFn) -> ItemFn {
        ...
        let fn_body = parse_quote! {
            {
                rticx_hippo::export::interrupt_disable();
                let r = f();
                unsafe { rticx_hippo::export::interrupt_enable(); } // critical section end
                r
            }
        };
    ...
    fn generate_resource_proxy_lock_impl(
        ...
        let lock_impl: syn::Block = parse_quote! {
            {
                unsafe { rticx_hippo::export::lock(resource_ptr, task_priority as u8, CEILING as u8, f) }
            }
        };
...
impl rticx_sw_pass::SwPassBackend for SwPassBackend {
    /// Path to the SPSC queue type re-exported by this distribution.
    fn queue_path(&self) -> Path {
        parse_quote!(rticx_hippo::export::Queue)
    }
    ...
    fn generate_local_pend_fn(&self, _core: u32, mut empty_body_fn: ItemFn) -> ItemFn {
        let body = parse_quote!({
            match irq_nbr {
                rticx_hippo::export::DispatcherIrq::Interrupt0 => {
                    rticx_hippo::export::pend(hippomenes_core::Interrupt0)
                }
                rticx_hippo::export::DispatcherIrq::Interrupt1 => {
                    rticx_hippo::export::pend(hippomenes_core::Interrupt1)
                }
                rticx_hippo::export::DispatcherIrq::Interrupt2 => {
                    rticx_hippo::export::pend(hippomenes_core::Interrupt2)
                }
                rticx_hippo::export::DispatcherIrq::Interrupt3 => {
                    rticx_hippo::export::pend(hippomenes_core::Interrupt3)
                }
            }
        });
    ...
    fn custom_interrupt_path(&self, _core: u32) -> Option<syn::Path> {
        Some(parse_quote!(rticx_hippo::export::DispatcherIrq))
    }
}
