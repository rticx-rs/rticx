...
impl CorePassBackend for AtalantaRtic {
        ...
        let init_dispatcher_interrupts =
            app_analysis.used_irqs.iter().map(|(irq_name, priority)| {
                ...
                quote! {
                    ...
                    rticx_atalanta::export::enable(
                        rticx_atalanta::export::interrupts::#irq_name,
                        #priority as u8,
                        #pcs
                    );
                }
    ...
    fn generate_interrupt_free_fn(&self, mut empty_body_fn: ItemFn) -> ItemFn {
        ...
        let fn_body = parse_quote! {
            {
                rticx_atalanta::export::interrupt_disable();
                let r = f();
                unsafe { rticx_atalanta::export::interrupt_enable(); } // critical section end
                r
            }
        };
    ...
    fn generate_resource_proxy_lock_impl(
        ...
        let lock_impl: syn::Block = parse_quote! {
            {
                unsafe { rticx_atalanta::export::lock(resource_ptr, task_priority as u8, CEILING as u8, f) }
            }
        };
...
impl rticx_sw_pass::SwPassBackend for SwPassBackend {
    ...
    fn queue_path(&self) -> Path {
        parse_quote!(rticx_atalanta::export::Queue)
    }
    ...
    fn generate_local_pend_fn(&self, _core: u32, mut empty_body_fn: ItemFn) -> ItemFn {
        let body = parse_quote!({
            rticx_atalanta::export::pend(irq_nbr);
        });
    ...
    fn custom_interrupt_path(&self, _core: u32) -> Option<syn::Path> {
        Some(parse_quote!(rticx_atalanta::export::Interrupt))
    }
}
