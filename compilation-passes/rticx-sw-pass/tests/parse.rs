//! Integration tests for the parsing phase of `rticx-sw-pass`.
//!
...
fn task_params_defaults() {
    let mut attr = sw_task_attr(quote!());
...
fn task_params_explicit_values() {
    let mut attr = sw_task_attr(quote!(priority = 3, core = 1, spawn_by = 0));
    let params = TaskParams::from_attr(&mut attr).expect("valid attr");
...
fn task_params_explicit_capacity() {
    let mut attr = sw_task_attr(quote!(capacity = 4));
    let params = TaskParams::from_attr(&mut attr).expect("valid attr");
...
fn task_params_shared_passthrough() {
    let mut attr = sw_task_attr(quote!(priority = 2, shared = [counter], init = generated));
    let params = TaskParams::from_attr(&mut attr).expect("valid attr");
    assert_eq!(params.priority, 2);
    // `shared` and `init` must survive so the core pass can see them.
    assert!(attr.elements.contains_key("shared"));
    assert!(attr.elements.contains_key("init"));
}
...
fn parse_task_attr_preserves_core_pass_args() {
    ...
    let items = quote! {
        #[sw_task(priority = 3, core = 0, spawn_by = 0, capacity = 2)]
        struct Foo;
    };
...
fn task_params_spawn_by_defaults_to_core() {
    let mut attr = sw_task_attr(quote!(core = 2));
    let params = TaskParams::from_attr(&mut attr).expect("valid attr");
...
fn task_params_non_integer_priority_errors() {
    let mut attr = sw_task_attr(quote!(priority = "high"));
    assert_err_contains(
...
fn task_params_overflowing_priority_errors() {
    let mut attr = sw_task_attr(quote!(priority = 99999999999));
    assert_err_contains(
        TaskParams::from_attr(&mut attr),
...
#[test]
fn parse_impl_for_non_sw_task_preserved() {
    // `impl RticSwTask for Bar` where `Bar` is not a software task must not
...
fn parse_qualified_trait_path_impl_matched() {
    let items = quote! {
        ...
        impl crate::app::RticSwTask for Foo {
            fn exec(&mut self) {}
        }
    };
    let app = parse_app(common::single_core_args(), items).expect("valid app");
...
fn parse_multiple_impls_for_same_task_errors() {
    ...
    assert_err_contains(
        parse_app(common::single_core_args(), items),
        "Multiple `RticSwTask` implementations found for task `Foo`",
    );
}

#[test]
fn parse_sw_task_on_enum_errors() {
    let items = quote! {
        #[sw_task]
        enum Foo {}
    };
