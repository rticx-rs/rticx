use crate::parse::ASYNC_TASK_TRAIT_TY;
...
/// Renames a parsed `#[async_task(...)]` attribute into the `#[task(...)]`
...
/// point the core pass at the async-task trait.  Any other key (e.g.
/// `shared`) is kept for the core pass.
...
fn sw_trait_ident() -> Ident {
    Ident::new(ASYNC_TASK_TRAIT_TY, Span::call_site())
}
