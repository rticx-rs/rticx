...
use crate::software_pass::parse::ast::AsyncTask;
use crate::software_pass::parse::{App, ASYNC_TASK_TRAIT_TY};
    ...
    fn generate_subapps(&mut self) -> TokenStream {
        ...
        let sub_apps = apps.zip(analysis).map(|(sub_app, sub_analysis)| {
            ...
            let sw_tasks = tasks_iter.map(|task| {
                // We will rename the "async_task" attribute to "task" so that the standard pass recognizes this as a task
                // also, we will add the `task_trait = RticAsyncTask` argument.
...
pub const SC_PEND_FN_NAME: &str = "__rticx_local_irq_pend"; // function name for core-local pending
pub const MC_PEND_FN_NAME: &str = "__rticx_cross_irq_pend"; // function name for cross-core pending
...
impl AsyncTask {
    ...
    fn generate_spawn_api(
        ...
        // get the inputs type. see the RticAsyncTask trait to understand this and where it comes from.
