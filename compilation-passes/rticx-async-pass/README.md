...
The pass uses a **type-witness pattern** to achieve monomorphized inline
future storage (zero per-spawn allocation). The technique is borrowed from
upstream RTIC's `from_ptr_n_args`.
...
3. **An `__rticx_init_async_slots()` function** — called from `post_init`,
   creates `ExecSlot<F>` on the heap once and stores the pointer:
   ```rust
   fn __rticx_init_async_slots() {
       let slot = Box::leak(Box::new(
           ExecSlot::new_from_witness(__rticx_async_Ping)
       ));
       PING_PTR.store(slot as *const _ as *const ());
   }
   ```
...
| Init-time allocation | One `Box::leak` per task (∼16–64 bytes each) |
| Heap required | Yes, for init-time `Box::leak` only; not for per-spawn |
| Stable Rust | Yes |
...
Async applications need `extern crate alloc;` at crate root (only for the
one-time `Box::leak` in the generated init function):
...
The wrapper-fn type-witness pattern would ideally create executors on `main()`'s
stack (which is `-> !` and lives forever), avoiding `Box` entirely.  However,
RTICX's `main()` is generated by the *core pass* after the async pass has
already run, so the async pass cannot inject locals into `main()`.  Until the
pass pipeline supports a post-core hook, the `Box::leak` bridge is practical:

- One heap allocation per task at init (not per spawn)
- No per-spawn allocation at all
- Monomorphized poll dispatch (no vtable)

This is a major improvement over the previous `Pin<Box<dyn Future>>` per-spawn
model, and the `Box::leak` can be eliminated when a post-core hook (or
`main()`-injection point) is available.  The init-time `Box::leak` overhead is
proportional to the number of *task definitions*, not the number of *spawns*.

### Why not TAIT / impl_trait_in_assoc_type?

These nightly features would allow naming the future type directly in a module
scope `type FooFuture = impl Future<Output = ()>;`, enabling a truly
heap-free `static mut SLOT: ExecSlot<FooFuture>`.  But:

- `type_alias_impl_trait` and `impl_trait_in_assoc_type` are both nightly-only
- Even with nightly, `async fn exec()` creates a *second* opaque type that
  cannot be unified with the TAIT — the user would need to write
  `fn exec() -> Self::Future { async move { ... } }` instead

The wrapper-fn witness approach avoids both issues and works on stable Rust.
...
- **Heap required for init** — one `Box::leak` per task definition at startup; see [Why not purely static?](#why-not-purely-static-allocation) for rationale. No per-spawn allocation.
