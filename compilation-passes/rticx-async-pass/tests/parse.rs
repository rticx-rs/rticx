...
// ---------------------------------------------------------------------------
// Block B : TaskParams::from_attr
// ---------------------------------------------------------------------------

fn async_task_attr(args: TokenStream) -> RticAttr {
    let s: syn::ItemStruct = syn::parse_quote! {
        #[async_task(#args)]
        struct Dummy;
    };
    RticAttr::parse_from_attr(&s.attrs[0]).expect("valid async_task attribute")
}
...
#[test]
fn parse_local_async_task_with_impl() {
    let items = quote! {
        #[async_task]
        struct Foo;
...
#[test]
fn parse_async_task_without_impl() {
    let items = quote! {
        #[async_task]
        struct Foo;
    };
...
#[test]
fn parse_custom_async_trait_name_recognized() {
    let items = quote! {
        #[async_task]
        struct Foo;

        impl MyRticAsyncTask for Foo {
            fn exec(&mut self) {}
        }
    };
    let app = parse_app(common::single_core_args(), items).expect("valid app");
...
#[test]
fn parse_non_async_structs_and_impls_go_to_rest() {
    let items = quote! {
        struct Bar;
...
fn parse_partition_local_vs_cross_core() {
    let items = quote! {
        ...
        impl RticAsyncTask for Local {
            fn exec(&mut self) {}
        }

        #[async_task(core = 1, spawn_by = 0)]
        struct Cross;
...
#[test]
fn parse_async_task_priority_param() {
    let items = quote! {
...
#[test]
fn parse_multiple_async_tasks_in_order() {
    let items = quote! {
        #[async_task]
        struct Foo;
        impl RticAsyncTask for Foo {
